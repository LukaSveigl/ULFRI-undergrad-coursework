...
# implementation of IDDFS (Iterative Deepening Depth-First Search) algorithm.
# It finds the path from a given start node, picks up all
# treasures and finds the exit using a stack.
# Each iteration, it is depth-limited based on size of labyrinth.
...
class IDDFS:
    ...
    def _write_results(self, cost, checked, route, labyrinth_number) -> None:
        ...
        file.write("Max depth:      " + str(self._max_depth))
    ...
    def _search(self, start_node, end_node, treasures) -> tuple:
        ...
        while depth_limit < len(self._labyrinth) * len(self._labyrinth[0]):
            # for depth_limit in range(len(self._labyrinth) * len(self._labyrinth[0])):
            self._max_depth = depth_limit
            ...
            while stack:
                ...
                if not found:
                    ...
                    for next_node in next_nodes:
                        x, y = next_node
                        if self._labyrinth[y][x] != -1 and next_node in previous.keys() and previous[next_node] == curr_node and curr_node in previous.keys() and previous[curr_node] != next_node:
                            visited[y][x] = False
