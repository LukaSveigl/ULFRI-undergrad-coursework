	...
	private AstExpr parseExprDisRst(AstExpr lExpr) {
		switch (currSymbol.token) {
			case OR -> {
				...
				AstExpr exprDis = parseExprDisRst(exprCon);
				return new AstBinExpr(s.location, AstBinExpr.Oper.OR, lExpr, exprDis);
			}
			default -> {
	...
	private AstExpr parseExprConRst(AstExpr lExpr) {
		switch (currSymbol.token) {
			case AND -> {
				...
				AstExpr exprCon = parseExprConRst(exprRel);
				return new AstBinExpr(s.location, AstBinExpr.Oper.AND, lExpr, exprCon);
			}
			default -> {
	...
	private AstExpr parseExprRelRst(AstExpr lExpr) {
		switch (currSymbol.token) {
			case EQU -> {
				...
				AstExpr exprRel = parseExprRelRst(exprAdd);
				return new AstBinExpr(s.location, AstBinExpr.Oper.EQU, lExpr, exprRel);
			}
			case NEQ -> {
				// Parse rest of relational expression, which is: != exprAdd exprRelRst
				...
				AstExpr exprAdd = parseExprAdd();
				AstExpr exprRel = parseExprRelRst(exprAdd);
				return new AstBinExpr(s.location, AstBinExpr.Oper.NEQ, lExpr, exprRel);
			}
			case GEQ -> {
				// Parse rest of relational expression, which is: >= exprAdd exprRelRst
				...
				AstExpr exprAdd = parseExprAdd();
				AstExpr exprRel = parseExprRelRst(exprAdd);
				return new AstBinExpr(s.location, AstBinExpr.Oper.GEQ, lExpr, exprRel);
			}
			case LEQ -> {
				// Parse rest of relational expression, which is: <= exprAdd exprRelRst
				...
				AstExpr exprAdd = parseExprAdd();
				AstExpr exprRel = parseExprRelRst(exprAdd);
				return new AstBinExpr(s.location, AstBinExpr.Oper.LEQ, lExpr, exprRel);
			}
			case LTH -> {
				// Parse rest of relational expression, which is: > exprAdd exprRelRst
				...
				AstExpr exprAdd = parseExprAdd();
				AstExpr exprRel = parseExprRelRst(exprAdd);
				return new AstBinExpr(s.location, AstBinExpr.Oper.LTH, lExpr, exprRel);
			}
			case GTH -> {
				// Parse rest of relational expression, which is: < exprAdd exprRelRst
				...
				AstExpr exprAdd = parseExprAdd();
				AstExpr exprRel = parseExprRelRst(exprAdd);
				return new AstBinExpr(s.location, AstBinExpr.Oper.GTH, lExpr, exprRel);
			}
			default -> {
	...
	private AstExpr parseExprAddRst(AstExpr lExpr) {
		switch (currSymbol.token) {
			case ADD -> {
				...
				AstExpr exprAdd = parseExprAddRst(exprMul);
				return new AstBinExpr(s.location, AstBinExpr.Oper.ADD, lExpr, exprAdd);
			}
			case SUB -> {
				// Parse rest of additive expression, which is: - exprMul exprAddRst
				...
				AstExpr exprMul = parseExprMul();
				AstExpr exprAdd = parseExprAddRst(exprMul);
				return new AstBinExpr(s.location, AstBinExpr.Oper.SUB, lExpr, exprAdd);
			}
			default -> {
	...
	private AstExpr parseExprMulRst(AstExpr lExpr) {
		switch (currSymbol.token) {
			case MUL -> {
				...
				AstExpr exprMul = parseExprMulRst(exprPref);
				return new AstBinExpr(s.location, AstBinExpr.Oper.MUL, lExpr, exprMul);
			}
			case DIV -> {
				// Parse rest of multiplicative expression, which is: / exprPref exprMulRst
				...
				AstExpr exprPref = parseExprPref();
				AstExpr exprMul = parseExprMulRst(exprPref);
				return new AstBinExpr(s.location, AstBinExpr.Oper.DIV, lExpr, exprMul);
			}
			case MOD -> {
				// Parse rest of multiplicative expression, which is: % exprPref exprMulRst
				...
				AstExpr exprPref = parseExprPref();
				AstExpr exprMul = parseExprMulRst(exprPref);
				return new AstBinExpr(s.location, AstBinExpr.Oper.MOD, lExpr, exprMul);
			}
			default -> {
	...
	private AstExpr parseExprPostRst(AstExpr lExpr) {
		switch (currSymbol.token) {
			case LBRACKET -> {
				...
				AstExpr post = parseExprPostRst(expr);

				return new AstBinExpr(s.location, AstBinExpr.Oper.ARR, lExpr, post);
			}
			case PTR -> {
				...
				AstExpr post = parseExprPostRst(lExpr);

				return new AstPstExpr(s.location, AstPstExpr.Oper.PTR, post);
			}
			default -> {
				//Report.info("exprPostRst -> ");
				return lExpr;
