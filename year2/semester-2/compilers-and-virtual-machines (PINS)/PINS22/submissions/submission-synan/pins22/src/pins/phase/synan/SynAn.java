...
public class SynAn implements AutoCloseable {
	...
	private void checkAndMove(Token t) {
		...
		if (currSymbol.token != t) {
			throw new Report.Error(currSymbol.location,
					"Syntax error! Expected: " + t + ", got: " + currSymbol.token);
		}
	 ...
	 *
	 * @throws Report.Error when current symbol not one of: VOID, CHAR, INT, PTR, LBRACKET, LPARENTHESIS.
	 */
	...
	/**
	 * Parse statement. This could be an IF statement, a WHERE statement, an assignment or expression statement.
	 */
