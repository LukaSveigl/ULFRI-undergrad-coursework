...
public class StmtGenerator implements AstVisitor<ImcStmt, Stack<MemFrame>> {
	...
	public ImcStmt visit(AstAssignStmt assignStmt, Stack<MemFrame> frames) {
		// Check if the assignment is to an expression with a memory location.
		if (!isLvalue(assignStmt.fstSubExpr)) {
			throw new Report.Error(assignStmt.location, "Cannot assign to non-lvalue.");
		}
	...
	/**
	 * The method that checks if an expression is an l-value.
	 *
	 * @param expr The expression to check.
	 * @return True if the expression is an l-value, false otherwise.
	 */
	private boolean isLvalue(AstExpr expr) {
		// If the expression is a variable, it is an l-value.
		if (expr instanceof AstNameExpr) {
			if (SemAn.declaredAt.get(expr) instanceof AstVarDecl) {
				return true;
			}
			else if (SemAn.declaredAt.get(expr) instanceof AstParDecl) {
				return true;
			}
		}
		// If the expression is a dereferenced pointer, it is an l-value.
		else if (expr instanceof AstPstExpr) {
			if (isLvalue(((AstPstExpr)expr).subExpr)) {
				return true;
			}
			return false;
		}
		// If the expression is an array access, it is an l-value.
		else if (expr instanceof AstBinExpr) {
			if (((AstBinExpr)expr).oper == AstBinExpr.Oper.ARR) {
				return true;
			}
		}
		return false;
