	...
	private AstExpr parseExprPostRst(AstExpr lExpr) {
		switch (currSymbol.token) {
			case LBRACKET -> {
				...
				return new AstBinExpr(s.location, AstBinExpr.Oper.ARR, lExpr, expr);
			}
			case PTR -> {
				...
				return new AstPstExpr(s.location, AstPstExpr.Oper.PTR, lExpr);
			}
			default -> {
				//Report.info("exprPostRst -> ");
				return lExpr;
	...
	private AstExpr parseIdentifierRst(AstNameExpr lExpr) {
		switch (currSymbol.token) {
			case LPARENTHESIS -> {
				...
				ASTs<AstExpr> exprs = new ASTs<>(s.location, parseIdentParams());
