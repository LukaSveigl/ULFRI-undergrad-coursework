...
public class LexAn implements AutoCloseable {
	 ...
	 *
	 * @param chr holds the whitespace character to resolve.
	 */
	...
	private boolean resolveWhitespaceCharacter() {
		...
		// This is needed that, for example "#char ax" throws error at "#char".
		if (getToken(lexeme) == null && commentCount == 0) {
	...
	private Symbol terminate() {
		...
		// end of lexeme, which would cause lexeme to not be committed.
		...
		// If entire file read, return EOF symbol. Location is twice ending column, as it only takes one character.
