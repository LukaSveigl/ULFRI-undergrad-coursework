...
def vsa_drevesa(gozd):
    return {(x, y) for y, g in enumerate(gozd) for x, el in enumerate(g) if el == "#"}


def stevilo_dreves(x0, y0, x1, y1, gozd):
    return len({(x, y) for y in range(y0, y1) for x in range(x0, x1) if gozd[y][x] == "#"})


def ni_dreves(x0, y0, x1, y1, gozd):
    return not any({True if je_drevo(x, y, gozd) else False for y, g in enumerate(gozd) for x, el in enumerate(g) if (min(x0, x1) <= x <= max(x0, x1)) and (min(y0, y1) <= y <= max(y0, y1))})


def dreves_na_33(x, y, gozd):
    return len({(yx, xx) for yx in range(y, y + 3) for xx in range(x, x + 3) if gozd[yx][xx] == "#"})


def naj_kvadrat(gozd):
    return max([dreves_na_33(x, y, gozd) for x in range(len(gozd[0]) - 2) for y in range(len(gozd) - 2)])


def lovec(x, y, pot, gozd):
    count = 0
    drevesa = vsa_drevesa(gozd)
    dimensions = dimenzije(gozd)
    for p in pot:
        if p == ">":
            if (x + 1, y) not in drevesa:
                if x + 1 < dimensions[0]:
                    x += 1
            else:
                count += 1
...
def zajec(drevesa):
    return max([(len(vals), vals[-1]) for k, vals in {x: sorted([(xx, yx) for xx, yx in drevesa if xx == x], key=lambda z: z[1]) for x, _ in drevesa}.items()])[1]
...
def bliznja_drevesa(x, y, r, drevesa):
    return {(xx, yx) for xx, yx in drevesa if razdalja(xx, yx, x, y) <= r and (xx, yx) != (x, y)}
...
def jasa(gozd):
    return max([(abs(x0 - x1) + 1) * (abs(y0 - y1) + 1) if x0 != x1 and y0 != y1 else (abs(y0 - y1) + 1) if x0 == x1 and y0 != y1 else (abs(x0 - x1) + 1) if x0 != x1 and y0 == y1 else 1 for (x0, y0), (x1, y1) in [((x0, y0), (x1, y1)) for x0, y0 in sorted(list({(x, y) for y, g in enumerate(gozd) for x, _ in enumerate(g)} - vsa_drevesa(gozd))) for x1, y1 in sorted(list({(x, y) for y, g in enumerate(gozd) for x, _ in enumerate(g)} - vsa_drevesa(gozd))) if ni_dreves(x0, y0, x1, y1, gozd)]])


def opica(x, y, r, drevesa):
    new_jumps = {d for d in drevesa if razdalja(x, y, d[0], d[1]) <= r}
    jumps = set().union(new_jumps)
    while new_jumps:
        new_jumps = set()
        for j in jumps:
            for d in drevesa:
                if razdalja(j[0], j[1], d[0], d[1]) <= r:
                    if d not in jumps:
                        new_jumps.add(d)
        jumps = jumps.union(new_jumps)
    return jumps
