...
def stevilo_okuzenih(oseba):
    ...
    if oseba not in stiki:
        return 1
    for k in stiki[oseba].keys():
        count += stevilo_okuzenih(k)
    return count + 1
...
def rekord(oseba):
    if oseba not in stiki:
        return 0
    max_infections = len(stiki[oseba])
    for k in stiki[oseba].keys():
        rec = rekord(k)
        if rec > max_infections:
            max_infections = rec
...
def nevarnost(oseba, razlog):
    ...
    if oseba not in stiki:
        return inf
    for pos, dej in stiki[oseba].items():
        if razlog in dej:
            inf += 1
        inf += nevarnost(pos, razlog)
    return inf
...
def okuzbe_zaradi(oseba, razlog):
    ...
    if oseba not in stiki:
        return inf
    for os, dej in stiki[oseba].items():
        if razlog in dej:
            inf += okuzbe_zaradi(os, razlog)
    return inf
...
def okuzbe_brez(oseba, prepovedane):
    ...
    for os, dej in stiki[oseba].items():
        for d in dej:
            if d not in prepovedane:
                inf += okuzbe_brez(os, prepovedane)
                break
    return inf
