...
def zrcaljen_navp(kos):
    return tuple((x for x in kos[::-1]))


def obrnjen(kos):
    chs = [[ch for ch in st] for st in kos]
    transp = []
    for i in range(len(chs[0])):
        new = []
        for j in chs:
            new.append(j[i])
        transp.append(''.join(new[::-1]))
    return tuple(transp)
...
def stranice(kos):
    stran = [kos[0], ''.join([st[-1] for st in kos]), kos[-1], ''.join([st[0] for st in kos])]
    stran += [el[::-1] for el in stran]
    return stran


def obrati(kos):
    obr = [zrcaljen_vod(kos), zrcaljen_navp(kos)]
    for i in range(4):
        obr.append(obrnjen_n(kos, i))
        obr.append(zrcaljen_navp(obr[-1]))
        obr.append(zrcaljen_vod(obr[-1]))
    return set(obr)
...
def zbirka_stranic(kosi):
    sides_list = [stranice(piece) for piece in kosi]
    sides = set([item for sub in sides_list for item in sub])
    zbirka = {side: {piece for piece in kosi if side in stranice(piece)} for side in sides}
    return zbirka
...
def kotni(kosi):
    ...
    fogli = set([])
    for kos in kosi:
        stran = stranice(kos)[:4]
        sum_len = 0
        for s in stran:
            sum_len += len(zbirka[s])
        if sum_len == 6:
            fogli.add(kos)
    return fogli
...
def prvi_kot(kosi):
    koti = list(kotni(kosi))
    first = sorted([s[0] for s in koti])
    najdu = ""
    for k in koti:
        if first[0] == k[0]:
            najdu = k
...
def vzemi_kos(levo, gor, po_stranicah, uporabljeni):
    if gor != 1 and levo != 1:
        kos = po_stranicah[gor] & po_stranicah[levo]
        if not kos:
            return None
        flip = preobrni(kos.pop(), levo, gor, po_stranicah)
        return flip
    elif gor != 1:
        kosi = po_stranicah[gor]
    elif levo != 1:
        kosi = po_stranicah[levo]
    for kos in kosi:
        flips = obrati(kos)
        for fl in flips:
            if fl in uporabljeni:
                break
        else:
            flip = preobrni(kos, levo, gor, po_stranicah)
            uporabljeni.add(flip)
            return flip
    return None
...
def sestavi_koscke(kosi, kos):
    ...
    len_puzzle = 0
    zbirka = zbirka_stranic(kosi)
    while len_puzzle != len(kosi):
        curr_sides = stranice(current_line[-1])[:4]
        if len(puzzle) == 0:
            new_piece = vzemi_kos(curr_sides[1], 1, zbirka, encountered)
        else:
            indices = len(current_line)
            if indices != len(puzzle[-1]):
                new_sides = stranice(puzzle[-1][indices])
            else:
                new_sides = stranice(puzzle[-1][indices - 1])
            new_piece = vzemi_kos(curr_sides[1], new_sides[2], zbirka, encountered)
        if new_piece is None:
            puzzle.append(current_line)
            new_sides = stranice(puzzle[-1][0])
            current_line = [vzemi_kos(1, new_sides[2], zbirka, encountered)]
        else:
            current_line.append(new_piece)

        len_puzzle = sum([len(x) for x in puzzle])
    return puzzle
...
def slika(kosi):
    ...
    for vrstica in kosi:
        vrstice = []
        for kos in vrstica:
            for i in range(1, len(kos) - 1):
                if len(vrstice) < i:
                    vrstice.append("")
                vrstice[i - 1] += kos[i][1:-1]
        for v in vrstice:
            image += v + "\n"
    return image.rstrip("\n")


def sestavljanka(kosi, kot):
    return slika(sestavi_koscke(kosi, kot))
